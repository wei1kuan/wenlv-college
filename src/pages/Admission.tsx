...
import { recruitNews, jobListings } from '@/data/mockData';
import type { NewsItem } from '@/data/mockData';

type ViewMode = 'card' | 'list';

const hashTabMap: Record<string, string> = {
  'admission-news': 'admission',
...
export default function Admission() {
  ...
  const [activeTab, setActiveTab] = useState(() => {
    ...
    return hashTabMap[hash] || 'admission';
  });
  ...
  const admissionItems: NewsItem[] = recruitNews.map((n) => ({ id: n.id, title: n.title, date: n.date, summary: n.summary }));
  const employmentItems: NewsItem[] = jobListings.map((j) => ({
    id: j.id + 100,
    title: `${j.title} — ${j.company}`,
    date: j.date,
    summary: `${j.location} | ${j.salary} | ${j.type}`,
  }));

  const tabs = [
    { key: 'admission', label: '招生动态', items: admissionItems },
    { key: 'employment', label: '就业信息', items: employmentItems },
  ];
            ...
            {viewMode === 'list' ? (
              ...
              <motion.div key="card" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.25 }}>
                <div className="flex gap-2 mb-6 border-b border-heritage-gold/20 pb-3">
                  {tabs.map((tab) => (
                    <button key={tab.key} onClick={() => setActiveTab(tab.key)} className={`px-5 py-2 text-sm font-medium rounded-t-lg transition-colors ${activeTab === tab.key ? 'bg-white text-heritage-primary border-t-2 border-heritage-primary -mb-[1px]' : 'text-gray-500 hover:text-heritage-primary hover:bg-heritage-cream/50'}`}>
                      {tab.label}
