...
import { industryNews } from '@/data/mockData';

type ViewMode = 'card' | 'list';

const hashTabMap: Record<string, string> = {
  'partner-companies': 'companies',
  'cooperation-projects': 'projects',
  'internship-bases': 'internship',
...
export default function Industry() {
  ...
  const [activeTab, setActiveTab] = useState(() => {
    ...
    return hashTabMap[hash] || 'companies';
  });

  useEffect(() => {
    const onHashChange = () => {
      const hash = window.location.hash.replace('#', '');
      if (hash && hashTabMap[hash]) setActiveTab(hashTabMap[hash]);
    };
  ...
  const tabs = [
    { key: 'companies', label: '合作企业', items: industryNews.companies },
    { key: 'projects', label: '合作项目', items: industryNews.projects },
    { key: 'internship', label: '实习基地', items: industryNews.internship },
    { key: 'innovation', label: '创新创业', items: industryNews.innovation },
  ];
          ...
          <AnimatePresence mode="wait">
            {viewMode === 'list' ? (
              ...
              <motion.div key="card" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.25 }}>
                <div className="flex gap-2 mb-6 border-b border-heritage-gold/20 pb-3 overflow-x-auto">
                  {tabs.map((tab) => (
                    <button key={tab.key} onClick={() => setActiveTab(tab.key)} className={`shrink-0 px-5 py-2 text-sm font-medium rounded-t-lg transition-colors ${activeTab === tab.key ? 'bg-white text-heritage-primary border-t-2 border-heritage-primary -mb-[1px]' : 'text-gray-500 hover:text-heritage-primary hover:bg-heritage-cream/50'}`}>
                      {tab.label}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                  {currentItems.map((item, i) => (
                    <motion.div key={item.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="bg-white rounded-xl shadow-sm hover:shadow-md border border-heritage-gold/10 overflow-hidden group transition-all">
                      ...
                      <div className="p-6">
                        ...
                        <div className="mt-3 pt-3 border-t border-gray-100 flex justify-end">
                          <span className="text-sm text-heritage-gold hover:text-heritage-primary transition-colors cursor-pointer">查看详情 →</span>
                        </div>
                      </div>
                    </motion.div>
                  ))}
                </div>
                {currentItems.length === 0 && <div className="py-16 text-center text-gray-400">暂无内容</div>}
              </motion.div>
            )}
          </AnimatePresence>
        </div>
