...
export default function Footer() {
  return (
    <motion.footer
      ...
      className="bg-heritage-blue text-white"
    >
      <div className="h-1 bg-heritage-gold" />
      ...
      <div className="container mx-auto px-4 py-12">
          ...
          <div>
            <div className="flex items-center gap-3 mb-4">
              <div className="w-12 h-12 bg-white rounded-lg flex items-center justify-center">
                <span className="text-heritage-red font-bold text-2xl">文</span>
              </div>
              <div>
            ...
            <ul className="space-y-2">
              {footerLinks.map((link) => (
                <li key={link.id}>
                  <a
                    href={link.path}
                    className="text-gray-300 hover:text-heritage-gold transition-colors duration-200 text-sm"
                  >
                    {link.name}
                  </a>
                </li>
            ...
            <ul className="space-y-3">
              {contactInfo.map((info) => (
                <li key={info.label} className="flex items-start gap-3 text-gray-300 text-sm">
                  <span className="text-heritage-gold">{info.icon}</span>
        ...
        <div className="mt-12 pt-8 border-t border-gray-700 text-center">
          <p className="text-gray-400 text-sm font-body">
            © 2024 文旅创意学院（荆楚非遗传承与创新实践中心） 版权所有
          </p>
        </div>
      </div>
    </motion.footer>
